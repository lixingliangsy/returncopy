...
/**
 * ReturnCopy Data Flywheel — writeback layer for the vertical return/refund-compliance agent.
 *
 * The curated consumer-protection dataset (eu / uk / us / frameworks) is a
 * static, vetted moat. The flywheel makes it *grow* over time: each agent run
 * emits candidate signals (new enforcement actions, uncertainty flags, citation
 * gaps, user feedback). These are written back to an append-only contributed
 * store, reviewed by a human, and — once accepted — merged into the retrievable
 * index on the next cold load.
 *
...
const ENF_RE =
  /([A-Z][A-Za-z0-9 .&'-]{2,40}?)\s+(?:was\s+)?(?:sued|settled|fined|penalized|sanctioned|ordered to refund)\s+.*?(?:\$|\$|USD|EUR|euro)\s?([\d.]+)\s?(M|million|k|thousand)?/i
...
const REF_RE =
  /\b(EU|Consumer Rights Directive|2011\/83|CRD|UK|Consumer Rights Act|2015|cra2015|CCR|Consumer Contracts|FTC|Cooling-Off|California|Automatic Renewal|ARL|Visa|Mastercard|chargeback|Art\.?\s*\d+|Cl\.?\s*\d+|Annex\s*[IVX]+|Directive\s*2011\/83)\b/gi
...
export function captureRunSignals(run: RunSignalInput): ContributedSignal[] {
  ...
  if (run.modelText && UNCERTAINTY_RE.test(run.modelText)) {
    out.push({
      ...
      text: 'Model output contained unresolved return/refund-compliance uncertainty requiring qualified-counsel confirmation.',
      source: 'agent-run',
      status: 'pending_review',
      createdAt: new Date().toISOString(),
      verify: true,
