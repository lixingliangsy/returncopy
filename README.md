...
> A return policy that keeps the trust (and the sale).
>
> Generate a clear, on-brand return/refund policy plus empathetic customer-facing replies for the most common return reasons.
...
```bash
npm run build
# Vercel: import repo, set framework = Next.js, root = 12_Micro_SaaS出海/returncopy
```
